...
| BUG-006 | API / manejador de errores | Media | P2 | Resuelto | `normalizeError` solo detectaba claves duplicadas (11000) cuando el error era `instanceof mongoose.MongooseError`; los errores del driver de MongoDB (`MongoServerError`), p. ej. en ejecuciones concurrentes de `create`, no cumplen esa condición y caían a `INTERNAL_ERROR` (500) en lugar de `409`. **Solución**: detectar `error.code === 11000` en cualquier objeto de error, antes del fallback. **Prueba**: `npm test` (137 casos) + revisión del manejador ✅ |
